...
QBCore.Functions.CreateUseableItem('licenseplate', function(source, item)
	TriggerClientEvent('aendir-licenseplates:client:applyplate', source, true)
end)

RegisterNetEvent('aendir-licenseplates:server:checkplateinfo', function (closevehicle, OriginalPlate)
	...
	if Plate then
		...
		TriggerClientEvent('aendir-licenseplates:client:setvehicleplate',PlayerID, closevehicle, NewPlate, OriginalPlate)
...
RegisterNetEvent('aendir-licenseplates:giveoldlicenseplate', function (OriginalPlate, NewPlate)
	local PlayerID = source
	local Player = QBCore.Functions.GetPlayer(PlayerID)
	local info = {}
